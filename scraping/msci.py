...
def sdg_to_dict(info_scraped):
    """
    Converts scraped Sustainable Development Goals (SDG) esg_company_data into a dictionary.

    Args:
        info_scraped (list): List of web elements containing SDG esg_company_data.

    Returns:
        dict: Dictionary with SDG esg_company_data.
    """
...
def controversies_to_dict(info_scraped):
    """
    Converts scraped controversies esg_company_data into a dictionary.

    Args:
        info_scraped (list): List of web elements containing controversies esg_company_data.

    Returns:
        dict: Dictionary with controversies esg_company_data.
    """
...
def decarbonization_target_to_dict(info_scraped):
    """
    Converts scraped decarbonization target esg_company_data into a dictionary.

    Args:
        info_scraped (list): List of web elements containing decarbonization target esg_company_data.

    Returns:
        dict: Dictionary with decarbonization target esg_company_data.
    """
...
def involvement_to_dict(info_scraped):
    """
    Converts scraped business involvement esg_company_data into a dictionary.

    Args:
        info_scraped (list): List of web elements containing business involvement esg_company_data.

    Returns:
        dict: Dictionary with business involvement esg_company_data.
    """
...
def scrape_msci(ticker):
    """
    Scrapes MSCI website for ESG-related esg_company_data based on the given ticker.
    ...
    try:
        # retrieve climate esg_company_data
    ...
    try:
        # retrieve business involvement esg_company_data
...
def update_all_companies_score():
    """
    Updates the ESG scores for all companies in the database that do not have SDG esg_company_data and have a valid ticker.

    This function fetches the necessary ESG data from the MSCI website and updates the company's esg_company_data in the
    MongoDB database.
    """
...
def add_into_mongodb(company_name, dec_dict, temp_goal, contr_dict, inv_dict, sdg_dict):
    """
    Updates MongoDB with ESG-related esg_company_data for a company.
